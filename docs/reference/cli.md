...
Complete reference for the `opencode-imperial` command-line interface.

## Basic Usage

```bash
# Display help
bunx opencode-imperial

# Or with npx
npx opencode-imperial
```
...
3. **Configuration File Creation**: Generates `opencode.json` or `opencode-imperial.json` files
4. **Plugin Registration**: Automatically registers the opencode-imperial plugin in OpenCode settings
...
Configuration
  ✓ opencode-imperial.json is valid
...
```bash
bunx opencode-imperial run [prompt]
```
...
bunx opencode-imperial mcp oauth login my-api --server-url https://api.example.com --client-id my-client --scopes "read,write"

# Remove stored OAuth tokens
bunx opencode-imperial mcp oauth logout <server-name>

# Check OAuth token status
bunx opencode-imperial mcp oauth status [server-name]
```
...
```bash
# Login
bunx opencode-imperial auth login

# Logout
bunx opencode-imperial auth logout

# Check current status
bunx opencode-imperial auth status
```
...
1. **Project Level**: `.opencode/opencode-imperial.json`
2. **User Level**: `~/.config/opencode/opencode-imperial.json`
...
bunx opencode-imperial doctor --verbose

# Check specific category only
bunx opencode-imperial doctor --category authentication
```
...
bunx opencode-imperial doctor --no-tui --json

# Save results to file
bunx opencode-imperial doctor --json > doctor-report.json
```
