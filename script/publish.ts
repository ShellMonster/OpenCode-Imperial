...
const PACKAGE_NAME = "opencode-imperial"
...
console.log("=== Publishing opencode-imperial (multi-package) ===\n")
...
async function updateAllPackageVersions(newVersion: string): Promise<void> {
  ...
  for (const platform of PLATFORM_PACKAGES) {
    const pkgName = `opencode-imperial-${platform}`
...
async function getContributors(previous: string): Promise<string[]> {
  ...
  const team = ["actions-user", "github-actions[bot]", "ShellMonster"]

  try {
    const compare =
      await $`gh api "/repos/ShellMonster/oh-my-opencode/compare/v${previous}...HEAD" --jq '.commits[] | {login: .author.login, message: .commit.message}'`.text()
...
async function gitTagAndRelease(newVersion: string, notes: string[]): Promise<void> {
  ...
  await $`git add package.json assets/opencode-imperial.schema.json`
  for (const platform of PLATFORM_PACKAGES) {
