...
| File | Purpose |
|------|---------|
| `add-plugin-to-opencode-config.ts` | Register `opencode-imperial` in `.opencode/opencode.json` plugin array |
...
| `generate-omo-config.ts` | Generate `opencode-imperial.jsonc` from install selections |
...
| `plugin-name-with-version.ts` | Resolve `opencode-imperial@X.Y.Z` for installation |
| `write-omo-config.ts` | Write generated config to `.opencode/opencode-imperial.jsonc` |
