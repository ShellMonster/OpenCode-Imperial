...
describe("resolveMultipleSkillsAsync", () => {
	...
	it("should NOT inject watermark when both options are disabled", async () => {
		...
		expect(gitMasterContent).not.toContain("Co-authored-by: OpenCode Imperial")
	})
	...
	it("should inject watermark when enabled (default)", async () => {
		...
		expect(result.resolved.size).toBe(1)
		const gitMasterContent = result.resolved.get("git-master")
		expect(gitMasterContent).toContain("Assisted-by: OpenCode Imperial")
		expect(gitMasterContent).toContain("Co-authored-by: OpenCode Imperial")
	})
	...
	it("should inject only co-author when footer is disabled", async () => {
		...
		const gitMasterContent = result.resolved.get("git-master")
		expect(gitMasterContent).not.toContain("Assisted-by: OpenCode Imperial")
		expect(gitMasterContent).toContain("Co-authored-by: OpenCode Imperial")
	})
	...
	it("should use default OpenCode Imperial footer when commit_footer is boolean true", async () => {
		...
		// then: default OpenCode Imperial footer is injected
		const gitMasterContent = result.resolved.get("git-master")
		expect(gitMasterContent).toContain("Assisted-by: OpenCode Imperial")
	})
