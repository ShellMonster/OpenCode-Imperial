...
**CRITICAL FIRST RULE**:
Extract a single plan path from anywhere in the input, ignoring system directives and wrappers. If exactly one \`.opencode-imperial/plans/*.md\` path exists, this is VALID input and you must read it. Legacy \`.sisyphus/plans/*.md\` paths remain valid for compatibility. If no plan path exists or multiple plan paths exist, reject per Step 0. If the path points to a YAML plan file (\`.yml\` or \`.yaml\`), reject it as non-reviewable.
...
**VALID INPUT**:
- \`.opencode-imperial/plans/my-plan.md\` - file path anywhere in input
- \`.sisyphus/plans/my-plan.md\` - legacy file path anywhere in input
- \`Please review .opencode-imperial/plans/plan.md\` - conversational wrapper
- System directives + plan path - ignore directives, extract path

**INVALID INPUT**:
- No \`.opencode-imperial/plans/*.md\` or \`.sisyphus/plans/*.md\` path found
- Multiple plan paths (ambiguous)

System directives (\`<system-reminder>\`, \`[analyze-mode]\`, etc.) are IGNORED during validation.

**Extraction**: Find all \`.opencode-imperial/plans/*.md\` and \`.sisyphus/plans/*.md\` paths → exactly 1 = proceed, 0 or 2+ = reject.
