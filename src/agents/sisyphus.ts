...
function buildDynamicSisyphusPrompt(
  ...
  return `<Role>
You are 太子(总管执行) - Chief Execution Coordinator in the 三省六部 system.

**Execution Doctrine**: Carry the work through to completion every day. Your output should be indistinguishable from a senior engineer's.
...
export function createSisyphusAgent(
  ...
  const base = {
    description:
      "Powerful AI orchestrator. Plans obsessively with todos, assesses search complexity before exploration, delegates strategically via category+skills combinations. Uses explore for internal code (parallel-friendly), librarian for external docs. (太子(总管执行) - 三省六部)",
    mode: MODE,
    model,
    maxTokens: 64000,
    prompt,
