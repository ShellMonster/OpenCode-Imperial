...
export async function downloadAstGrep(version: string = DEFAULT_VERSION): Promise<string | null> {
  ...
  if (!platformInfo) {
    log(`[${PLUGIN_PACKAGE_NAME}] Unsupported platform for ast-grep: ${platformKey}`)
    return null
  }
  ...
  log(`[${PLUGIN_PACKAGE_NAME}] Downloading ast-grep binary...`)
  ...
  try {
    ...
    log(`[${PLUGIN_PACKAGE_NAME}] ast-grep binary ready.`)

    return binaryPath
  } catch (err) {
    log(
      `[${PLUGIN_PACKAGE_NAME}] Failed to download ast-grep: ${err instanceof Error ? err.message : err}`
    )
    return null
  }
}
