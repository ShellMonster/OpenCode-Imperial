...
describe("findPluginEntry", () => {
  ...
  test("returns unpinned for bare package name", () => {
    ...
    fs.writeFileSync(configPath, JSON.stringify({ plugin: ["opencode-imperial"] }))
  ...
  test("returns unpinned for latest dist-tag", () => {
    ...
    fs.writeFileSync(configPath, JSON.stringify({ plugin: ["opencode-imperial@latest"] }))
  ...
  test("returns unpinned for beta dist-tag", () => {
    ...
    fs.writeFileSync(configPath, JSON.stringify({ plugin: ["opencode-imperial@beta"] }))
  ...
  test("returns pinned for explicit semver", () => {
    ...
    fs.writeFileSync(configPath, JSON.stringify({ plugin: ["opencode-imperial@3.5.2"] }))
